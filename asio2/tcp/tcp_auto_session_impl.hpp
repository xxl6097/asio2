...
#ifndef __ASIO2_TCP_AUTO_SESSION_IMPL_HPP__
#define __ASIO2_TCP_AUTO_SESSION_IMPL_HPP__
...
namespace asio2
{
	...
	class tcp_auto_session_impl : public tcp_session_impl
	{
	...
	public:
		 ...
		 * @construct
		 */
		explicit tcp_auto_session_impl(
	...
	protected:
			...
			if (this->is_started())
			{
				if (m_recv_is_header)
				{
					...
					asio::async_read(this->m_socket,
						...
						this->m_strand_ptr->wrap(std::bind(&tcp_auto_session_impl::_handle_recv, this,
		...
		virtual void _post_send(std::shared_ptr<session_impl> this_ptr, std::shared_ptr<buffer<uint8_t>> buf_ptr) override
		{
			...
			if (is_started())
			{
				...
				tcp_session_impl::_post_send(std::move(this_ptr), std::move(buf_ptr));
			}
			else
			{
...
#endif // !__ASIO2_TCP_AUTO_SESSION_IMPL_HPP__
