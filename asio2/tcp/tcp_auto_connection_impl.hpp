...
#ifndef __ASIO2_TCP_AUTO_CONNECTION_IMPL_HPP__
#define __ASIO2_TCP_AUTO_CONNECTION_IMPL_HPP__
...
namespace asio2
{
	...
	class tcp_auto_connection_impl : public tcp_connection_impl
	{
	public:
		/**
		 * @construct
		 */
		explicit tcp_auto_connection_impl(
		 ...
		 * @destruct
		 */
		virtual ~tcp_auto_connection_impl()
		{
		}
	...
	protected:
			...
			if (this->is_started())
			{
				if (m_recv_is_header)
				{
					...
					asio::async_read(this->m_socket,
						...
						this->m_recv_strand_ptr->wrap(std::bind(&tcp_auto_connection_impl::_handle_recv, this,
		...
		virtual void _post_send(std::shared_ptr<connection_impl> this_ptr, std::shared_ptr<buffer<uint8_t>> buf_ptr) override
		{
			...
			if (is_started())
			{
				...
				tcp_connection_impl::_post_send(std::move(this_ptr), std::move(buf_ptr));
			}
			else
			{
...
#endif // !__ASIO2_TCP_AUTO_CONNECTION_IMPL_HPP__
